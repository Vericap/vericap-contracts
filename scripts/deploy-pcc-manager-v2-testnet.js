...
async function main() {
	...
	let VericapPCCManager
	let vericapPCCManager
	try {
		await hre.run('compile')
		VericapPCCManager = await hre.ethers.getContractFactory('VericapPCCManager')
		vericapPCCManager = await upgrades.deployProxy(
			VericapPCCManager,
			[process.env.ADMIN_WALLET_ADDRESS],
			{ kind: 'uups' }
		)
		await vericapPCCManager.deployed()
	} catch (err) {
